# Bootstrap 5 5.3.8 × Grounded UI dialog contract 0.3.0

Generated by `node reports/run.mjs bootstrap`. 4 of 4 published examples contained a dialog; axe reported 1 violation(s) across them. A live docs page can hold several instances; each is checked.
...
| Rule | Level | Failing examples | Of those, axe silent | Example detail |
| --- | --- | ---: | ---: | --- |
| DG-01 The root is a <dialog> element. | recommended | 4 | 3 | root matches :scope:not(dialog) |
...
| DG-07 A trigger on the page points at the dialog with commandfor and command="show-modal". | recommended | 4 | 3 | nothing on the page matches [command="show-modal"][commandfor="exampleModalLive"] |
| DG-08 The trigger is a <button>. | recommended | 0 | — |  |
| DG-09 The dialog contains a close control (command="close" or a form with method="dialog"). | recommended | 4 | 3 | root matches :scope:not(:has([command="close"], form[method="dialog"])) |
| DG-10 The dialog is not rendered with a static open attribute; it opens as a modal via show-modal. | recommended | 0 | — |  |
| DG-12 The dialog itself has no tabindex. | recommended | 4 | 3 | root matches :scope[tabindex] |
...
- docs: live demo, opened
- docs: static backdrop, opened
- docs: scrollable, opened
- docs: form in modal, opened
